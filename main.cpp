...
auto CallHacks()->VOID
{
	while (true)
	{
		if (CFG.b_NoRecoil)
		{
			...
			write<float>(GameVars.ranged_weapon_component + 0x1F0, 0.f);

		}
		if (CFG.b_Fastreload)
		{
			auto RangedWeaponComponent = read<DWORD_PTR>(GameVars.local_player_pawn + 0x4c8);//UTigerRangedWeaponComponent

			write<float>(RangedWeaponComponent + 0x198, 5.f);//CurrentFireRateMultiplier
			write<float>(GameVars.local_player_pawn + 0xb90 + 0x4c, 10.f);//ReloadSpeedMultiplier
	...
	for (int index = 0; index < EntityList_Copy.size(); ++index)
	{
		...
		if (CFG.b_Visual)
		{
			if (entity_distance < CFG.max_distance)
			{
				...
				{
					char dist[64];
					sprintf_s(dist, "Dist1111:%.fm", entity_distance);
				...
				if (CFG.b_checkteam)
				{
					auto CurrentGroupID = read<DWORD_PTR>(GameVars.local_player_state + 0x3E0);
					auto GroupID = read<DWORD_PTR>(Entity.actor_state + 0x3E0);
...
void Render()
{
	...
	if (CFG.b_MenuShow)
	{
		...
		ImGui::Begin("King FaHad || AL3QRB // test 1.2 ", 0, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar);
		...
		if (CFG.tab_index == 0)
		{
			...
			if (CFG.b_Visual)
			{
				...
				ImGui::Checkbox("ESP PlayerLevel", &CFG.b_PlayerLevel);
				ImGui::Checkbox("ESP checkteam", &CFG.b_checkteam);
		...
		else if (CFG.tab_index == 2)
		{
			...
			ImGui::Checkbox("Fastreload", &CFG.b_Fastreload);
		}
		ImGui::PopFont();
		ImGui::End();
	}
